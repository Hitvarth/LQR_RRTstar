...
public:
	...
	// discrete time ricatti equation  : https://wikimedia.org/api/rest_v1/media/math/render/svg/f34676c09a3e240fa782e85db974a7e42a7d50ee

	float** transpose(float mx[2][])
	{
		float tr[2][2];
	...
	float** multiply_matrices(float a[2][], float b[2][])     //used the 
	{
		// float** c;
		float** c= new float*[2];
		c[0]= new float[2];
	...
	float** inverse(float mx[1][1])   /// we would need inverse of a 1x1 matrix
	{
		float inv[1][1];
		inv[0][0]=1/mx[0][0];
		return inv
	...
	float** add_matrices(float a[2][2],float b[2][2])
	{
		float res[2][2];
	...
	float** subtract_matrices(float a[2][2],float b[2][2])    // a-b
	{
		for(int i=0;i<2;i++)
			for(int j=0;j<2;j++)
				b[i][j]=-b[i][j];
	...
	float max_element(float mx[2][2])
	{
		float max=-999;
		for(int i=0;i<2;i++)
			for (int j = 0; j<2; j++)
	...
	float** abs_element(float mx[2][2])
	{
		for(int i=0;i<2;i++)
			for(int j=0;j<2;j++)
				if(mx[i][j]<0)
	...
	float** LQR_control(float A[2][2],float B[2][1],float x[2][1])
	{
		float Q[2]][2]={{1,0},{0,1}}, R[1][1]={{1}}, X[2][2], Xn[2][2];
		float EPS=0.01;
		...
		{
			Xn =subtract_matrices( add_matrices( multiply_matrices( multiply_matrices(transpose(A), X), A ), Q) ,
			                                     multiply_matrices(
			                       	                                 multiply_matrices( multiply_matrices( multiply_matrices( transpose(A), X), B ) , inverse( add_matrices( multiply_matrices( multiply_matrices( transpose(B), X), B ), R) ),
			                       	                                 multiply_matrices( multiply_matrices( transpose(B), X ), A )	
			                        	                          )   
			                                   )	                                                                                   
			                       
			                     )
			if( max( abs_element(subtract_matrices(Xn,X))) < EPS )
				break;
		...
		K = multiply_matrices( inverse( add_matrices( multiply_matrices( transpose(B),multiply_matrices( Xn, B)), R )), multiply_matrices( transpose(B), multiply_matrices( Xn, A ) ) )

		float minus_K[1][2], u[1][1];
		for(int j=0;j<2;j++)
			minus_K[0][j]=-1*K[0][j]

		u = multiply_matrices(minus_K,x);
	...
	Node* my_new_node(Node* nearest,Node* N)
	{
		Node* new_node = new Node
		float s= stepsize;
		// Point m;
		...
		float A[2][2]={{DT,1.0},{0.0,DT}},B[2][1]={{0.0},{1.0}};
		...
		while(time<=MAX_TIME)
		{
			...
			x = add_matrices(multiply_matrices(A,x),multiply_matrices(B,u));
